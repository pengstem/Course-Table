...
function normalizeWeeksRule(rule) {
    ...
    const skipWeeks = Array.isArray(source.skipWeeks)
        ? source.skipWeeks.map(item => ensureInt(item, -1)).filter(item => item > 0)
        : [];

    const extraWeeks = Array.isArray(source.extraWeeks)
        ? source.extraWeeks.map(item => ensureInt(item, -1)).filter(item => item > 0)
        : [];
...
function normalizeException(item) {
    ...
    if (action === 'override') {
        ...
        normalized.endPeriod = Math.max(normalized.startPeriod, ensureInt(source.endPeriod, normalized.startPeriod));
    }
    ...
    if (Array.isArray(schedule.courses)) {
        schedule.courses.forEach((course, index) => {
            ...
            if (!weeksRule) {
                ...
                if (!Number.isInteger(weeksRule.endWeek) || weeksRule.endWeek < weeksRule.startWeek)
                    errors.push(`${prefix}.weeksRule.endWeek must be >= startWeek.`);

                if (!PARITY_VALUES.has(weeksRule.parity))
                    errors.push(`${prefix}.weeksRule.parity must be one of all/odd/even.`);
            }
        });
    }
