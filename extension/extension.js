...
function buildAllCourseRows(schedule, periodTimes) {
  ...
  const rows = courses.map((course) => {
    const startText =
      periodTimes[String(course.startPeriod)]?.start ??
      `P${course.startPeriod}`;
    const endText =
      periodTimes[String(course.endPeriod)]?.end ?? `P${course.endPeriod}`;
...
function courseStatus(occurrence, now) {
  ...
  if (
    nowMs >= occurrence.startDateTime.getTime() &&
    nowMs < occurrence.endDateTime.getTime()
  )
    return "active";

  if (nowMs < occurrence.startDateTime.getTime()) return "upcoming";
...
const CourseIndicator = GObject.registerClass(
  class CourseIndicator extends PanelMenu.Button {
    ...
    _createSectionHeading(text, iconName, colorClass) {
      ...
      if (iconName) {
        const iconClasses = ["ct-section-heading-icon"];
        if (colorClass) iconClasses.push(colorClass);
        box.add_child(
          new St.Icon({
            icon_name: iconName,
    ...
    _createCourseRow(occurrence, now) {
      ...
      // Name column (expands)
      const nameClass =
        status === "active" ? "ct-name-col-active" : "ct-name-col";
      row.add_child(
        new St.Label({
          text: occurrence.name,
          style_class: nameClass,
          x_expand: true,
          y_align: Clutter.ActorAlign.CENTER,
        }),
      );
    ...
    _createSearchRow(row) {
      ...
      rowBox.add_child(
        new St.Label({
          ...
          style_class: `ct-weekday-badge ct-weekday-${row.weekday}`,
          y_align: Clutter.ActorAlign.CENTER,
        }),
      );
    ...
    _createSummaryBar(stats) {
      ...
      bar.add_child(
        new St.Label({
          text: String(stats.totalCourses),
          style_class: "ct-summary-num",
        }),
      );
      bar.add_child(
        new St.Label({
          text: " courses  \u00B7  ",
          style_class: "ct-summary-text",
        }),
      );

      const conflictClass =
        stats.todayConflicts > 0
          ? "ct-summary-conflict"
          : "ct-summary-conflict-zero";
      bar.add_child(
        new St.Label({
          text: String(stats.todayConflicts),
          style_class: conflictClass,
        }),
      );
      bar.add_child(
        new St.Label({
      ...
      bar.add_child(
        new St.Label({
          text: `${weekdayLabel(stats.busiestWeekday)}`,
          style_class: "ct-summary-num",
        }),
      );
      bar.add_child(
    ...
    _createActionButtons() {
      ...
      const refreshBox = new St.BoxLayout({
        style_class: "ct-action-btn",
        x_align: Clutter.ActorAlign.CENTER,
      });
      refreshBox.add_child(
        new St.Icon({
          icon_name: "view-refresh-symbolic",
          style_class: "ct-action-btn-icon",
        }),
      );
      refreshBox.add_child(
        new St.Label({
          text: "Refresh",
          style_class: "ct-action-btn-label",
          y_align: Clutter.ActorAlign.CENTER,
        }),
      );
    ...
    _filteredRows(rows) {
      if (!this._searchText) return rows.slice(0, 8);
    ...
    render(state) {
      ...
      else this._label.set_text("No Class");

      // ── Next Class section ──
      this.menu.addMenuItem(
        this._createSectionHeading(
          "Next Class",
          "alarm-symbolic",
          "ct-section-icon-next",
        ),
      );
      ...
      this.menu.addMenuItem(
        this._createSectionHeading(
          `Today (${todaySummary.count})`,
          "view-list-symbolic",
          "ct-section-icon-today",
        ),
      );
...
export default class CourseTableExtension extends Extension {
  enable() {
    ...
    this._reminderScheduler = new ReminderScheduler((title, body) =>
    ...
    this._settingsSignals = [
      this._settings.connect("changed::schedule-file-path", () =>
        this._refreshView(),
      ),
      this._settings.connect("changed::period-times-json", () =>
        this._refreshView(),
      ),
      this._settings.connect("changed::week1-start-date", () =>
        this._refreshView(),
      ),
    ];
  ...
  disable() {
    ...
    if (this._settingsSignals && this._settings) {
      this._settingsSignals.forEach((signalId) =>
  ...
  _refreshView() {
    ...
    const periodTimes = parsePeriodTimesFromSettings(
      this._settings.get_string("period-times-json"),
    );
